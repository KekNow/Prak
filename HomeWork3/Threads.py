...
if __name__ == '__main__':
    ...
    path = "Tests_Audio2"
    destination = path + "_RESULTS_THREADS"
    with ThreadPoolExecutor(4) as pool:
        for way, folders, files in os.walk(path):
            dest = destination + "\\" + way[len(path) + 1:]
            os.mkdir(dest)
            if files:
                pool.submit(extract_mfcc, way, destination + "\\" + way[len(path)+1:], files)
    with open("Results.txt", 'a') as res:
        timer = "Working time, threads: " + str(time.perf_counter() - start_time) + '\n'
        res.write(timer)